...
std::string EC2Engine::sign(const std::string& secret,
			    const std::string& string_to_sign) const
{
  ...
  */
  ...
  if (lines.size() < 1) {
    ldout(cct, 0) << "String to sign is not in any recognisable format" << dendl;
    throw -ERR_SIGNATURE_NO_MATCH;
  }

  if (lines[0] != "AWS4-HMAC-SHA256") {
    ldout(cct, 0) << "Signature format " << lines[0] << "cannot be handled internally" << dendl;
    throw -ERR_SIGNATURE_NO_MATCH;
  }

  if (lines.size() < 4) {
    ldout(cct, 0) << "String to sign does not contain all necessary data for AWS4-HMAC-SHA256 signing mode" << dendl;
    throw -ERR_SIGNATURE_NO_MATCH;
  }
  ...
  if (scope_data.size() < 4) {
    ldout(cct, 0) << "String to sign does not contain all necessary credential scope data for AWS4-HMAC-SHA256 signing mode" << dendl;
    throw -ERR_SIGNATURE_NO_MATCH;
  }
  ...
  if (command_name != "aws4_request") {
    ldout(cct, 0) << "credential_scope is not for aws4_request when using AWS4-HMAC-SHA256 signing mode" << dendl;
    throw -ERR_SIGNATURE_NO_MATCH;
  }
...
std::pair<boost::optional<std::string>, int> EC2Engine::get_secret_from_keystone(const std::string& user_id, const boost::string_view& access_key_id) const
{
  ...
  ret = secret.process();
  if (ret < 0) {
    ldout(cct, 2) << "s3 keystone: secret fetching error: "
                  << token_body_bl.c_str() << dendl;
    return make_pair(boost::none, ret);
  }
  ...
  if (! parser.parse(token_body_bl.c_str(), token_body_bl.length())) {
    ldout(cct, 0) << "Keystone credential parse error: malformed json" << dendl;
    return make_pair(boost::none, -EINVAL);
  }
  ...
  try {
    if (!credential_iter.end()) {
      ...
      ldout(cct, 0) << "Keystone credential not present in return from server" << dendl;
      return make_pair(boost::none, -EINVAL);
    }
  } catch (JSONDecoder::err& err) {
    ldout(cct, 0) << "Keystone credential parse error: " << err.message << dendl;
    return make_pair(boost::none, -EINVAL);
  }
 ...
 */
std::pair<boost::optional<rgw::keystone::TokenEnvelope>, int>
EC2Engine::get_access_token(const boost::string_view& access_key_id,
                            const std::string& string_to_sign,
                            const boost::string_view& signature) const
{
  ...
  if (t) {
    ...
    std::string calculated_signature = sign(t->get<1>(), string_to_sign);
    if (sig == calculated_signature) {
      return std::make_pair(t->get<0>(), 0);
    } else {
      ldout(cct, 0) << "Secret string does not correctly sign payload, cache miss" << dendl;
    }
  } else {
    ldout(cct, 0) << "No stored secret string, cache miss" << dendl;
  }

  /* No cached token, token expired, or secret invalid: fall back to keystone */
  std::tie(token, failure_reason) = get_from_keystone(access_key_id, string_to_sign, signature);

  if (token) {
    /* Fetch secret from keystone for the access_key_id */
    boost::optional<std::string> secret;
    std::tie(secret, failure_reason) = get_secret_from_keystone(token->get_user_id(), access_key_id);
  ...
  const req_state* s) const
{
  ...
  std::tie(t, failure_reason) = \
    get_access_token(access_key_id, string_to_sign, signature);
  if (! t) {
